...
export const useSignalStore = defineStore('signal', () => {
    ...
    function play() {
        if (!result.value || totalSlices.value === 0)
            return;
    ...
    const currentSlice = computed(() => isReplaying.value && result.value ? result.value.waterfall[playIndex.value] ?? null : null);
    ...
    const visibleWaterfall = computed(() => {
        if (!result.value)
            return [];
    ...
    const visibleSpectrum = computed(() => {
    ...
    const visibleConstellation = computed(() => {
        const res = result.value;
        if (!res)
            return [];
        if (!isReplaying.value)
            return res.constellation;
